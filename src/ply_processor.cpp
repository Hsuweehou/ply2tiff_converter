/**
 * @file ply_processor.cpp
 * @brief L系列线扫相机 Qt 处理器实现
 * 
 * 实现 L系列线扫相机的Qt多线程处理逻辑，包括：
 * - PLY文件属性读取
 * - 固定宽度点云转换
 * - 灰度属性选择和通道顺序处理
 */
...
void PlyProcessor::process(const std::string& inputPath, int imgWidth, const std::string& grayPropertyName, int channelOrder)
{
    emit logMessage("开始处理路径: " + QString::fromStdString(inputPath));
    emit logMessage("图像宽度设置为: " + QString::number(imgWidth));
    emit logMessage("通道顺序: " + QString::fromStdString(channelOrder == 0 ? "OpenCV格式 (ZYX)" : "Open3D格式 (XYZ)"));
    ...
    for (int j = 0; j < ply_file_paths.size(); j++) {
        ...
        save_tiff(ply_file_paths[j], output_path + "\\" + ply_files[j] + "_xyz.tiff", output_path + "\\" + ply_files[j] + "_gray.tiff", imgWidth, grayPropertyName, channelOrder);
...
void PlyProcessor::save_ply2tiff(std::vector<float>& x_coords, std::vector<float>& y_coords, std::vector<float>& z_coords,
                                 int width, int height, const std::string& filename, int channelOrder) {
    ...
    if (channelOrder == 0) {
        ...
        for (int row = 0; row < height; ++row) {
            for (int col = 0; col < width; ++col) {
                ...
                pixel[0] = x_coords[idx];  // Z -> 通道0 (B)
                pixel[1] = y_coords[idx];  // Y -> 通道1 (G)
                pixel[2] = z_coords[idx];  // X -> 通道2 (R)
            }
        }
    } else {
        // Open3D格式: XYZ顺序
        for (int row = 0; row < height; ++row) {
            for (int col = 0; col < width; ++col) {
                int idx = row * width + col;
                cv::Vec3f& pixel = tiff_image.at<cv::Vec3f>(row, col);
                pixel[0] = z_coords[idx];  // Z -> 通道0 (R)
                pixel[1] = y_coords[idx];  // Y -> 通道1 (G)
                pixel[2] = x_coords[idx];  // X -> 通道2 (B)
            }
        }
    }
...
int PlyProcessor::save_tiff(const std::string& filename, const std::string& ply_tiff, const std::string& gray_tiff, int img_width, const std::string& grayPropertyName, int channelOrder) {
    ...
    //emit logMessage("++++++++++++++++++++++");
    save_ply2tiff(plys_x, plys_y, plys_z, img_width, plys_z.size() / img_width / 2, ply_tiff, channelOrder);
